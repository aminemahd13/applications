...
3.  **Microsite image optimization**:
    -   New microsite image uploads are compressed on upload (best effort).
    -   Existing microsite images are backfilled by a scheduler.
    -   Asset keys remain unchanged; format is preserved (`jpg/png/webp` stay in the same format).
...
#### Microsite Media Optimization Tuning (Optional)

| Variable | Description |
|----------|-------------|
| `MICROSITE_MEDIA_OPTIMIZATION_TARGET_BYTES` | Best-effort target size for optimized microsite images (default `819200`). |
| `MICROSITE_MEDIA_OPTIMIZATION_MAX_EDGE_PX` | Maximum image edge after resize (default `1920`). |
| `MICROSITE_MEDIA_OPTIMIZATION_BATCH_SIZE` | Number of pending microsite image jobs claimed per scheduler batch (default `8`). |
| `MICROSITE_MEDIA_OPTIMIZATION_MAX_BATCHES_PER_RUN` | Upper bound on optimization batches handled per cron execution (default `4`). |
| `MICROSITE_MEDIA_OPTIMIZATION_MAX_ATTEMPTS` | Retry cap before marking optimization as failed (default `3`). |
